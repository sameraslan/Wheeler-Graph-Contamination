...
USAGE = '''usage: DeBruijnGraph_generator.py [-h / --help] [-v/ --version] [-o / --ofile FILE] [-k / --kmer k-mer length] [-l / --seqLen sequence length] [-a / --alnNum alignment number] sequence FASTQ files'''

# We loop through all the FASTQ files and construct the graph
def main(argv):
    k = 3
    verbose = False
    outfile = "tmp.dot"
    ...
    if len(args) < 1:
        print(USAGE)
        print("Please input at least one FASTQ file")
        sys.exit(2)
...
def process_fasta_file(fasta_file, k, seqLen, alnNum, organism_id, kmer_dic):
    ...
    for node_kmer in kmer_set:
        if node_kmer in kmer_dic:
            kmer_dic[node_kmer].add_organism_id(organism_id)
        else:
