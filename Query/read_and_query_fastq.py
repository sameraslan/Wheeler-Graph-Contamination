...
PATTERN = "GATCCAAA" # S1 -> S3 -> S6 -> S4 -> S8 -> S7
...
def query_graph(G, pattern):
    ...
    while starting_edge + current_pattern_idx < len(dfs_edges):
        ...
        if char == pattern[current_pattern_idx]:
...
def parse_label(label):
    label = label.strip('"')
    char, ids = label.split(' ')
    ids = ids.strip("()").strip('[]]').split(',')
    return char, [int(id) for id in ids]
